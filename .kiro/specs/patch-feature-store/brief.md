...
- 正常パッチ特徴の追記だけで更新でき、勾配更新は不要（researches.md §11）。
  初期構築と定期追加が同じ登録操作で行える。データセット由来の既知正常
  （VisA の `train/good` 等）は HITL 検証を介さない初期構築経路として一括登録できる
  （`docs/visa-validation-gate.md`）。
- coreset 再選択でサイズ上限を維持し、古い・冗長・失効（expiry）したプロトタイプを
  間引ける（researches.md §11）。
- 評価用に、指定した由来キー（ウェハ ID・ロット ID・撮像日）の集合だけを含む固定サイズの
  バンクを複数構築できる。正常のみデータでの安定性検証が、同一画像をそれを含まない複数バンクに
  対してスコア化することを要求するため（`docs/normal-only-validation-plan.md`）。
...
- 登録は検証済みの正常のみとし、HITL 検証・スキーマ検証・監査ログで正常性が担保された
  データだけを受け入れて汚染を防ぐ（researches.md §11）。データセットのラベルで正常性が
  担保される初期構築では、担保の根拠（データセット名・split）を登録時に記録して代替する。

## Scope

- **In**: インデックス構築・近傍検索・増分追加・永続化、ドメイン分割、coreset 再選択、
  expiry 間引き、検証済み正常のみを登録するガード、データセット由来の既知正常による
  初期構築経路、メタデータ（annotation・構造化 JSON・適用メタ情報）の索引。
