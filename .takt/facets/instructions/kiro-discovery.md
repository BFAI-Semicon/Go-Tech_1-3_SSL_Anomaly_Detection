...
6. roadmap の checklist marker は spec readiness だけを表す。新規または未承認 spec は `[ ]` とし、`[x]` は `spec.json.phase` が `tasks-generated` または後続の `implemented`、requirements/design/tasks approvals、`ready_for_implementation == true`、required artifacts の存在を確認できる場合だけ使う。
