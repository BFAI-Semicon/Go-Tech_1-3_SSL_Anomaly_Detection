...
5. acceptance criteria は `rules/ears-format.md` の EARS fixed phrase に沿って書く。trigger keyword と fixed phrase（`When`、`If`、`then`、`While`、`Where`、`shall`）は英語のまま保持し、可変部（イベント・前提・条件・主体・応答）だけを対象言語で書く。trigger や fixed phrase の内部に対象言語のテキストを混入させない。
   - `When [イベント], the [システム] shall [応答]`
   - `While [前提], the [システム] shall [応答]`
   - `If [条件/異常], then the [システム] shall [応答]`
   - `Where [機能/オプション], the [システム] shall [応答]`
   - `The [システム] shall [応答]`
