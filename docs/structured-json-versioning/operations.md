...
- 稼働系は起動時に `active-manifest` が指すマニフェストのタプル（**全ドメイン**）をロードし、**ドメイン
  （工程・材料・装置）をキーにしたインメモリ索引**を構築する（線形スキャン禁止）。索引には各具体ドメインの
  specificity チェーン（`any`・上位工程クラスの広域ドメインを含む §4.2 の合成順）を前計算して張り、クロスドメイン
  競合（§9.1）を解けるようにする。
- これにより、マッチ計算はファイル全体サイズではなく「該当ドメイン＋その specificity チェーンの有効ルール数」に
  比例し、全ドメインをロードしてもファイルが大きくなるほどには検索性能が劣化しない。
...
- anomalib は PyPI の `anomalib[cu130]>=2.6,<3`（DINOv3 は `TimmFeatureExtractor` 経由。
  特徴抽出のみ利用）。
