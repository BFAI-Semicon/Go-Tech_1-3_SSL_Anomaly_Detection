...
- `element_id`: 論理要素の安定 ID（JSON 整数。編集は同一 ID で新 `revision`）。**全ドメインで一意**
  （稼働状態あたり全体1ファイルの `priority.json` §9.1 がこれをキーに順序付けるため）。
  単一ライタがドメイン横断のグローバル連番で新規要素にのみ採番し、`Retire` 後も再利用しない。
  ID の大小に政策的意味はない（意図的な順序は `priority.json`）
...
5. **最終タイブレーク**：なお同点なら **`element_id` が大きい方を優先**（数値比較。総順序を保証し一意化）。
   ...
   順序は `priority.json` で明示する（Retire して新規採番し直すことで勝ちに行く運用はしない）。
