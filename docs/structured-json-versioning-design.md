...
- **検出時のスコープ**：ドメインで正常集合を絞る場合は、**`domain_id` 集合がクエリの具体ドメインを含む
  プロトタイプ**を用いる（集約点は複数ドメインのクエリに正当に効く）。汎用なのは特徴抽出器（凍結 SSL、
  再学習なし）であり、全ドメインをプールした汎用検出も可能。ドメイン分割は分布が乖離する場合の較正手段
  （`researches.md §3.3・§8`）で、モデルを分ける意味ではない。
